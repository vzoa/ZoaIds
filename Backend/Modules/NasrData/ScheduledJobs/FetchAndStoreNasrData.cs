...
namespace ZoaIdsBackend.Modules.NasrData.ScheduledJobs;
...
public class FetchAndStoreNasrData : IInvocable
{
    ...
    public FetchAndStoreNasrData(ILogger<FetchAndStoreNasrData> logger, IDbContextFactory<ZoaIdsContext> contextFactory, HttpClient httpClient, IOptionsMonitor<AppSettings> appSettings)
    {
        _logger = logger;
    ...
    public async Task Invoke()
    {
        ...
        try
        {
            ...
            var lastFetch = await db.CompletedJobs
                .AsNoTracking()
                .Where(j => j.Caller == typeof(FetchAndStoreNasrData).Name && j.JobKey == "fetch" && j.Status == ApplicationJobRecord.ExitStatus.Success)
    ...
    private async Task<string> GetNasrZipUrl()
    {
        ...
        using var apiStream = await _httpClient.GetStreamAsync(_appSettings.CurrentValue.Urls.NasrApiEndpoint);
    ...
    private static Airport.AirportType StringToAirportType(string code)
    {
        return code switch
        {
            "A" => Airport.AirportType.Airport,
            "B" => Airport.AirportType.Ballonport,
            "C" => Airport.AirportType.SeaplaneBase,
            "G" => Airport.AirportType.Gliderport,
            "H" => Airport.AirportType.Heliport,
            "U" => Airport.AirportType.Ultralight,
    ...
    private static (string, Runway.End) ParseCsvRowToRunwayEnd(CsvReader csv)
    {
        var runwayEnd = new Runway.End
        {
