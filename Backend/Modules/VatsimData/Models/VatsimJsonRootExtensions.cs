...
public static class VatsimJsonRootExtensions
{
    ...
    public static IEnumerable<VatsimJsonController> GetCabControllersByPrefix(this VatsimJsonRoot vatsimJsonRoot, string airportPrefix)
    {
        var towerFacility = vatsimJsonRoot.Facilities.Where(f => f.Short == "TWR").FirstOrDefault();
        if (towerFacility is null)
        {
            return Enumerable.Empty<VatsimJsonController>();
        }

        return vatsimJsonRoot.Controllers.Where(c => c.Facility <= towerFacility.Id && c.Callsign.StartsWith(airportPrefix, StringComparison.OrdinalIgnoreCase));
    }
